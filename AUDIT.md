...
# Production Readiness Assessment (2026-06-11)

A point-in-time review of whether the application is ready for production deployment,
conducted after all 57 first- and second-pass findings were remediated.

## What Is Solid

| Area | State |
|---|---|
| **Authentication** | Entra ID only, HttpOnly cookies, CSRF rotation at login/logout, refresh token revocation (JTI + reuse detection) |
| **Authorization** | Backend-enforced on every route; no raw group IDs or Graph payloads in API responses |
| **Input validation** | Zod at every request boundary; `sortBy` whitelisted via `z.enum`; CSV formula injection neutralized |
| **Security headers** | Helmet with explicit CSP, body size limits (`1 MB`), general (500 req/15 min) and auth-specific (20 req/15 min) rate limiting |
| **Audit trail** | Immutable `audit_log` table for financially significant operations (PO approval, invoice waiver, bulk delete) |
| **Graceful shutdown** | SIGTERM/SIGINT handled; HTTP server drained before exit; 10 s force-exit fallback |
| **Structured logging** | Winston with daily rotation; prod/dev format switching; no stack traces in production responses |
| **Env validation** | `validateEnv()` fails fast at startup on missing required vars |
| **SSL/TLS** | Let's Encrypt + Certbot auto-renewal wired into `docker-compose.yml` |
| **Database migrations** | `prisma migrate deploy` on container start; 72 migrations tracked; migration SQL committed alongside schema changes |
| **Error handling** | Global error handler; internal details not leaked in production responses |
| **Health checks** | Public `/health` (status only); admin-authenticated `/health/details` |
| **DB connection pool** | Pool configured with `max`, `idleTimeoutMillis`, `connectionTimeoutMillis`; pool-level errors logged |
| **Performance** | N+1 query fixes; pagination on location/room inventory; PDF download single-query |

## Gaps That Remain

These findings are **not regressions** from prior audits ? they were out of scope for the security/quality passes. They are new findings against the production-readiness bar.

---

### ~~PR-1~~ ✅ — ~~No Automated Tests~~

**Severity:** ~~🔴~~ High

Zero test files exist in the backend (`backend/src/**/*.test.ts` ? no matches). The preflight script (`scripts/preflight.ps1`) validates that TypeScript compiles and Vite builds, but it does not exercise any runtime behaviour ? permission checks, service logic, Zod validation paths, or API contract correctness.

A regression in any controller, service, or middleware would ship silently. All 57 security fixes have no regression protection.

**Fix:** Add integration tests (Vitest + Supertest against a test database) covering at minimum: authentication flows, CSRF enforcement, permission gate rejection, and the highest-risk fixed paths (JTI revocation, sortBy whitelist, location-scope enforcement on work orders). Use `npx vitest run` (not watch mode ? see FORBIDDEN COMMANDS).
...
### PR-3 ? No CI/CD Pipeline

**Severity:** ?? Medium

There are no GitHub Actions workflows (`.github/workflows/` ? no files). Every deployment is manual. There is no automated preflight gate on pull requests ? a developer can push broken TypeScript or a failed build directly to `master`.

**Fix:** Add a GitHub Actions workflow that runs `scripts/preflight.ps1` (or the equivalent Docker image builds) on every push to `master` and on every pull request. Gate merges on a green build.
...
`validateEnv()` references `.env.example` in its error message, but the file does not exist at the repo root. A fresh deployment requires reverse-engineering required variables from `docker-compose.yml`, `validateEnv.ts`, and the codebase. This increases deployment error risk and onboarding time.

**Fix:** Create `.env.example` with every `process.env.*` variable used in the codebase, each with a description comment and a placeholder value. Ensure it matches the variables already listed in `docker-compose.yml` and `validateEnv.ts`, including the optional SMTP and scheduler vars called out in LOW-6.

---

### PR-5 ? No External Monitoring or Alerting

**Severity:** ?? Medium

Logs are written to rotating files inside Docker volumes (`backend_logs`). Nothing ships logs to an external aggregator, and there is no uptime alerting. If the backend container crashes or hangs at 2 AM, there is no notification path. The `/health` endpoint exists but nothing polls it externally.

**Fix:** At minimum: configure an external uptime monitor (UptimeRobot, Better Uptime, or similar) to ping `/health` every 5 minutes and alert on failure. For log visibility, ship container logs to a managed service (Papertrail, Logtail, CloudWatch) or mount a log volume that is accessible outside the container.
...
Six Entra group IDs (`VITE_ENTRA_*_GROUP_ID`) are passed as build-time args and compiled into the React bundle. They are visible to anyone who inspects the compiled JavaScript. Authorization is correctly enforced on the backend, so this is not exploitable ? but it exposes internal Azure AD tenant structure, and a group ID change requires a full frontend rebuild and redeploy.

This was flagged as ARCH-2 in the first pass and partially addressed (authorization logic moved to backend). The group IDs remain in the bundle for UI convenience flags. Accept as a known, documented tradeoff, or remove them and derive all UI flags from the `/api/auth/me` response instead.
...
Recommended remediation order: **PR-2 ? PR-5 ? PR-4 ? PR-3 ? PR-1**